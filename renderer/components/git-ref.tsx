...
function GitRef(props: GitRefProps) {
  function getElement(ref: string) {
    if (ref.includes('origin/HEAD')) {
      ...
      return <span className='whitespace-nowrap w-fit border-red-500 bg-red-900 rounded-lg pl-2 pr-2 mr-1'>{ref}</span>;
    } else if (ref.includes('HEAD ->')) {
      // Local HEAD
      const branch = ref.match(/HEAD -> (.*)/)[1];
      return (
        <span className='whitespace-nowrap w-fit border-green-500 bg-green-900 rounded-lg pl-2 pr-2 mr-1'>
      ...
      // Tag
      return (
        <span className='whitespace-nowrap w-fit border-blue-500 bg-blue-900 rounded-lg pl-2 pr-2 mr-1'>
          {ref.match(/tag: (.*)/)[1]}
        </span>
      );
    } else if (ref != '') {
      // Local branch
      return (
        <span className='whitespace-nowrap w-fit border-green-500 bg-green-900 rounded-lg pl-2 pr-2 mr-1'>{ref} </span>
      );
    } else {
      // no ref
      return null;
