    ...
    if (ref.includes('origin/HEAD')) {
      ...
      return (
        <span className='flex h-full border border-green-300 bg-green-900 rounded px-2 mr-1'>
          <img className='max-w-none' width='12px' height='12px' src='/images/git-branch.svg' />
        </span>
      );
    } else if (ref.includes('origin')) {
      // Remote branch
      return (
        <span className='whitespace-nowrap flex h-full w-fit border border-green-300 bg-green-900 rounded text-sm px-2 mr-1'>
          {ref}
        </span>
      );
      ...
      return (
        <span className='whitespace-nowrap flex h-full w-fit border border-red-300 bg-red-900 rounded text-sm px-2 mr-1'>
          {branch}
        </span>
      );
      ...
      // Tag
      return (
        <span className='whitespace-nowrap flex h-full w-fit border border-blue-300 bg-blue-900 rounded text-sm px-2 mr-1'>
          {ref.match(/tag: (.*)/)[1]}
        </span>
      );
    } else if (ref != '') {
